...
export const PokemonFilter: React.FC<PokemonFilterProps> = memo(({ onFilter }) => {
  ...
  return (
    <div data-name='PokemonFilter' className='flex flex-wrap justify-between px-2 my-2 bg-gray-100 rounded'>
      ...
      {filteredPokemons.length > 0 && (
        <div className='flex items-center -mx-2'>
          <div className='p-2'>{filteredPokemons.length} {filteredPokemons.length > 1 ? 'Pokemons' : 'Pokemon'} found</div>
          <button className='link p-2' onClick={resetFilter}>Reset</button>
        </div>
      )}
    </div>
  )
